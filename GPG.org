...
2. Replace all occurences of this key with a place holder:
   #+begin_src sh
     ...
     git ls-files | xargs sed -i 's|ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIPP0GqblNA1rwWKsKNnTZPNmh5cU4IEGGBdiVDF0w7gJ.*"|gpg_rotate_holder"|g'
   #+end_src
3. In password store =$HOME/.password-store=, decrypt all passwords to
   ...
   #+begin_src sh
     ...
     cd $HOME
   #+end_src
4. Delete existing GNUPGHOME:
   #+begin_src
     rm -rf $(readlink $HOME/.gnupg)
   #+end_src
5. Generate a new GPG key with =gpg --gen-key=.  Defaults are good
   enough.  Choose a strong password.
6. Add two subkeys.  One for SSH (Authenticate), another one for
   signing (Sign).  Run =gpg --expert --edit-key yuchen@apvc.uk=, then
   enter =addkey= at the prompt.  Follow on screen instructions.
...
8. Add SSH key keygrip to =$HOME/.gnupg/sshcontrol=
   #+begin_src
     ...
     E5FDC0E7EF673F37F06626426D2E4F941DB8EB29
   #+end_src
9. Make a tarball of GNUPGHOME:
   #+begin_src sh
   tar --auto-compress --create --file gpg-with-master-key.tar.xz $HOME/.gnupg
   #+end_src
...
11. Create another tarball for backup:
    #+begin_src sh
      tar --auto-compress --create --file gpg.tar.xz $HOME/.gnupg
    #+end_src
12. Back up both tarballs:
    #+begin_src sh
      scp gpg.tar.xz gpg-with-master-key.tar.xz tl.yc:~/
    #+end_src
13. Update gpg key id file of the password store:
     #+begin_src
       echo "742FB9A3903E8AE7D88CF89EE4B9611E05059668" > $HOME/.password-store/.gpg-id
     #+end_src
14. Encrypt passwords with new key and delete plain text files:
    #+begin_src sh
      cd $HOME/.password-store
...
15. Commit changes and push:
    ...
    #+end_src
16. Show new SSH public key:
    #+begin_src sh
      [yc@qinghe:~]$ gpg --export-ssh-key yuchen@apvc.uk
      ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIPP0GqblNA1rwWKsKNnTZPNmh5cU4IEGGBdiVDF0w7gJ openpgp:0xCBE44FB8
    #+end_src
17. Replace place holder in config with new key:
   #+begin_src sh
     cd $HOME/nixos-config
   ...
   #+end_src
18. Commit changes and push:
    #+begin_src sh
      pass git commit -asm'rotate ssh key'
    #+end_src
19. Update key on Github and other web services.
20. Upload key to [[https://keys.openpgp.org/][key server]].
21. To restore GNUPGHOME, use:
    #+begin_src sh
