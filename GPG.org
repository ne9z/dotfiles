...
1. Replace all occurences of this key with a place holder:
   #+begin_src sh
     ...
     git ls-files | xargs sed -i "s|$(gpg --export-ssh-key yuchen@apvc.uk)|gpg_rotate_holder|g"
   #+end_src
...
13. Replace place holder in config with new key:
   #+begin_src sh
     cd $HOME/nixos-config
     git ls-files | xargs sed -i "s|gpg_rotate_holder|$(gpg --export-ssh-key yuchen@apvc.uk)|g"
   #+end_src
...
19. Restore new home:
    ...
    #+end_src
20. Export new public and ssh key:
    #+begin_src sh
      gpg --export --armor --output pubkey.asc
      gpg --export-ssh-key yuchen@apvc.uk
    #+end_src
21. Upload key to [[https://keys.openpgp.org/][key server]].
22. Update pubkey and ssh key on Github and other web services.
23. Appendix: To restore GNUPGHOME, use:
    #+begin_src sh
